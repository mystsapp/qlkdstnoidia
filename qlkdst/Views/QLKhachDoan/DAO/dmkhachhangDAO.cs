...
namespace qlkdstDB.DAO
{
    public class dmkhachhangDAO
    {
        ...
        public List<DDLDmKH> GetDDLDmKHByTenGiaoDich(string tengiaodich, string suserId, string role, string schinhanh)
        {
            ...
            string sQuery = "  SELECT makh,tengiaodich+'-'+isnull(diachi,'') tengiaodich FROM[dbo].[dmkhachhang] where (tengiaodich like '%" + tengiaodich + "%' OR tenthuongmai like '%" + tengiaodich + "%' OR makh like '%" + tengiaodich + "%' OR makhold like '%" + tengiaodich + "%')  ";
        ...
        {
            List<dmkhachhang> model = db.dmkhachhang.Where(x => x.makh == makh || (x.makhold == makh && x.makhold != null)).ToList();
