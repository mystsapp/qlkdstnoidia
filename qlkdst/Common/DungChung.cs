...
namespace qlkdst.Common
{
    ...
    public class DungChung
    {
        ...
        public static List<SelectListItem> LayDSNganhNghe()
        {
            qlkdtrEntities db = new qlkdtrEntities();
            List<dmnghanhnghe> lstNN = db.dmnghanhnghe.ToList();
            ...
            foreach (dmnghanhnghe cn in lstNN)
            {
                SelectListItem item = new SelectListItem();


                item.Text = cn.tennghanhnghe;
                //item.Value = cn.loaitourid.ToString();
                item.Value = cn.tennghanhnghe;
                lst.Add(item);
            }
